...
# ── Auto-manage recruiting visibility ────────────────────────────────────────
# Visibility is entirely system-controlled: True when solo, False in a team.
# Signals fire on Team/TeamMember create/delete and keep profiles in sync
...
def _sync_visibility_for_user(user):
    """Set profile.visibility = True iff the user is not in any team."""
    profile = getattr(user, 'participant_profile', None)
    if profile is None:
        return
    is_in_team = (
        Team.objects.filter(leader=user).exists() or
        TeamMember.objects.filter(email=user.email).exists()
    )
    expected = not is_in_team
    if profile.visibility != expected:
        profile.visibility = expected
        profile.save(update_fields=['visibility'])


def _sync_visibility_by_email(email):
    User = get_user_model()
    user = User.objects.filter(email=email).first()
    if user:
        _sync_visibility_for_user(user)


@receiver(post_save, sender=Team)
@receiver(post_delete, sender=Team)
def _on_team_change(sender, instance, **kwargs):
    """Recalculate visibility for the team leader and all its members."""
    _sync_visibility_for_user(instance.leader)
    for email in TeamMember.objects.filter(team=instance).values_list('email', flat=True):
        _sync_visibility_by_email(email)


@receiver(post_save, sender=TeamMember)
@receiver(post_delete, sender=TeamMember)
def _on_member_change(sender, instance, **kwargs):
    """Recalculate visibility for the affected member."""
    _sync_visibility_by_email(instance.email)
