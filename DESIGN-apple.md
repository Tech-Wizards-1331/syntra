...
## Selective Glassmorphism (VisionOS / Monterey Style Extension)
*Added during Apple Light Glass Migration phase*

While the core system relies on solid white (`{colors.canvas}`) and parchment (`{colors.canvas-parchment}`) surfaces, **frosted glass** is used as a strategic depth layer inspired by visionOS and macOS. 

### Glass Philosophy
- **Accent, not Surface:** Glass is used strategically to create depth (like VisionOS or Apple Music). It does not replace the primary white productivity surfaces.
- **Productivity First:** Forms, tables, and data-heavy grids MUST remain pure white and high-contrast (`bg-canvas`). Do not sacrifice readability for aesthetics.
- **Subtle Mesh:** A barely-noticeable mesh gradient (`.apple-mesh-bg`) provides refraction for the glass without distracting from content. It uses extremely low-opacity soft pinks, blues, and purples on top of `{colors.canvas-parchment}`.

### Glass Application Matrix
| Component Type | Treatment | CSS Utility | Blur Radius |
|---|---|---|---|
| **Global Navigation / Sidebars** | Frosted Glass | `.apple-glass-nav` | 20px |
| **Quick Action / Metric Cards** | Frosted Glass | `.apple-glass-card` | 20px |
| **Modals / Drawers** | Heavy Frosted Glass | `.apple-glass-modal` | 30px |
| **Data Tables / Complex Forms** | Solid White (No Glass) | `bg-canvas` | 0px |
| **Registration Flows** | Solid White (No Glass) | `bg-canvas` | 0px |

**Performance constraints:** Do not apply backdrop blur to every card in a large grid. Reserve glass for singular hero cards, floating elements, navigation, and modals.
